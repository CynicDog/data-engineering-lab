...
Key patterns demonstrated:
    ...
    Silver. To require ALL four tables first, use AssetAlias or a sensor.
...
@dag(
    ...
    schedule=BRONZE_ASSETS,  # Triggered when ANY bronze table is refreshed
    catchup=False,
    max_active_runs=1,
...
def transform_silver():
    ...
    def transform_all() -> dict:
        ...
        from lakehouse.silver.transform import TABLES, transform_table
        ...
        try:
            for table in TABLES:
                rows = transform_table(spark, settings, table)
                results[table] = rows
        finally:
            spark.stop()
