...
at a file on the disk. By mapping a file to the kernel's page cache, the system eliminates the need for expensive
system calls and data copying, treating the file as if it were already in the computer's memory. Multiple processes
can map the same file simultaneously, providing a high-performance way to share data through a single physical
memory space.
